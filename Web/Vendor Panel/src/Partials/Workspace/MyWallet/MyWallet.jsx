...
const MyWallet = () => {
  ...
  setPageTitle("Payments | Vendor Panel");
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`${APP_URL}/vendor-order-totals`, {
          headers: {
        ...
        handleApiError(error, "fetching", "vendor order totals");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `${APP_URL}/vendor-order-totals/${date}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        handleApiError(error, "fetching", "vendor order totals by date");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`${APP_URL}/all-vendor-payments`, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
        ...
        handleApiError(error, "fetching", "vendor payments");
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="card bg-transparent border-0">
        <div className="card-header">
          ...
          <h3
            ...
            style={{ padding: "15px", fontSize: "14px", fontWeight: "bold" }}
          >
